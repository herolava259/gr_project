...
from keras.layers import BatchNormalization, Conv3D, Activation
...
class DCN(object):
    ...
    def _coordinate_map_3D(self, offset_field):
        ...
        x_offset = tf.squeeze(x_offset, [4])  # [N,H,W,D,3*3*3]
        y_offset = tf.squeeze(y_offset, [4])
        z_offset = tf.squeeze(z_offset, [4])
        ...
        x_center = tf.transpose(
            tf.reshape(x_center, [self.height, self.depth, self.width]), [0, 2, 1]
        )
        x_center = tf.reshape(x_center, [self.height * self.width * self.depth, -1])
        x_center = tf.tile(x_center, [1, self.num_points])
        x_center = tf.reshape(
            x_center, [self.height, self.width, self.depth, self.num_points]
        )
        ...
        y_center = tf.transpose(
            tf.reshape(y_center, [self.width, self.depth, self.height]), [2, 0, 1]
        )
        y_center = tf.reshape(y_center, [self.height * self.width * self.depth, -1])
        y_center = tf.tile(y_center, [1, self.num_points])
        y_center = tf.reshape(
            y_center, [self.height, self.width, self.depth, self.num_points]
        )
        ...
        z_center = tf.transpose(
            tf.reshape(z_center, [self.height, self.width, self.depth]), [0, 1, 2]
        )  # actually no transpose
        z_center = tf.reshape(z_center, [self.height * self.width * self.depth, -1])
        z_center = tf.tile(z_center, [1, self.num_points])
        z_center = tf.reshape(
            z_center, [self.height, self.width, self.depth, self.num_points]
        )
        ...
        # regular grid
        x = tf.linspace(
            float(-(self.kernel_size[0] - 1) / 2),
            float((self.kernel_size[0] - 1) / 2),
            self.kernel_size[0],
        )
        y = tf.linspace(
            float(-(self.kernel_size[1] - 1) / 2),
            float((self.kernel_size[1] - 1) / 2),
            self.kernel_size[1],
        )
        z = tf.linspace(
            float(-(self.kernel_size[2] - 1) / 2),
            float((self.kernel_size[2] - 1) / 2),
            self.kernel_size[2],
        )
        ...
        x_grid = tf.reshape(
            x_grid, [self.height, self.width, self.depth, self.num_points]
        )
        y_grid = tf.tile(y_spread, [1, self.height * self.width * self.depth])
        y_grid = tf.reshape(
            y_grid, [self.height, self.width, self.depth, self.num_points]
        )
        z_grid = tf.tile(z_spread, [1, self.height * self.width * self.depth])
        z_grid = tf.reshape(
            z_grid, [self.height, self.width, self.depth, self.num_points]
        )
        ...
        x_new = tf.reshape(
            tf.transpose(x_new, [0, 1, 4, 2, 5, 3, 6]),
            [
                self.num_batch,
                self.kernel_size[0] * self.height,
        ...
        y_new = tf.reshape(
            tf.transpose(y_new, [0, 1, 4, 2, 5, 3, 6]),
            [
                self.num_batch,
                self.kernel_size[0] * self.height,
                self.kernel_size[1] * self.width,
        ...
        z_new = tf.reshape(
            tf.transpose(z_new, [0, 1, 4, 2, 5, 3, 6]),
            [
                self.num_batch,
                self.kernel_size[0] * self.height,
                self.kernel_size[1] * self.width,
    ...
    def _bilinear_interpolate_3D(self, input_feature, x, y, z):
        ...
        input_feature_flat = tf.reshape(
            input_feature, tf.stack([-1, self.num_channels])
        )  # [N*H*W*D,C]
            ...
            tf.expand_dims(
                tf.ones(
                    shape=(
                        tf.stack(
                            [
                                self.num_points * self.height * self.width * self.depth,
                            ]
                        )
                    )
                ),
